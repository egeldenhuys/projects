...
Module Module1
    ...
    ' Variables
    Dim CurrentVersion As String = "1.2.0"
    ...
    Sub Main()
        ...
        ' This is better than a "Program Fun With Cars.exe has stopped working" window popping up.
        ' It also provides information on the error.
        Try
            ' Set up the backgroundWorker used to check for updates, and start it.
    ...
    Private Sub SetupColors()
        ...
        cInstruction = ConsoleColor.Magenta ' "Please enter the information as required."
        cInfo = ConsoleColor.Blue ' "Car has been added"
...
#Region "Update Functions"
    ...
    ByVal e As DoWorkEventArgs) Handles bgWorker.DoWork
    ...
    Private Sub bgWorker_ProgressChanged(ByVal sender As System.Object, _
    ByVal e As ProgressChangedEventArgs) Handles bgWorker.ProgressChanged
    ...
    ''' <summary>
    ''' This sub downloads the latest version string from the internet and stores it in the LatestVersion variable
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub GetLatestVersion()

        ' Create the request
        Dim request As HttpWebRequest = CType(WebRequest.Create("https://dl.dropboxusercontent.com/u/57022080/Programs/Fun%20With%20Cars/version.txt"), HttpWebRequest)
        ...
        LatestVersion = readStream.ReadToEnd()
    ...
    ''' <summary>
    ''' This function gets the latest version and compares it with the current version.
    ''' </summary>
    ''' <returns>Returns True when there is a new version available, otherwise False.</returns>
    ''' <remarks></remarks>
    Private Function NewVersionAvailable() As Boolean
    ...
    ''' <summary>
    ''' Prints the current version to the bottom of the screen
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub PrintVersion()

        ' Save the cursor position before the options are printed.
        Dim CursorPos() As Integer = {Console.CursorLeft, Console.CursorTop}
    ...
    ''' <summary>
    ''' This screen gives the user a choice of updating.
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub ScreenUpdateChoice()
        ' We only want to force this screen to appear once.
        UpdateShown = True

        ' Initialize screen
        Console.Clear()
        WriteHeader("Update Available")
        Console.WriteLine()

        ' Display option
        cWriteArray("A new version is available. (Version: |" & LatestVersion & "| )", {cInfo, ConsoleColor.Green, cInfo})
        cWriteArray("Do you want to download it now (recommended)?| [|Y|/|N|]", {cInstruction, cDeco, cOption, cDeco, cOption, cDeco})

        ' Wait for user input
        Dim input As ConsoleKeyInfo = Console.ReadKey(True)

        If input.Key = ConsoleKey.Y Then
            ScreenDownloadUpdate()
        End If

    End Sub

    ''' <summary>
    ''' This screen checks for an update. If there is one it shows the choice screen.
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub ScreenCheckingForUpdate()

        WriteHeader("Checking For New Version")
        Console.WriteLine()

        cWrite("Checking if a new version is available, please wait...", cInstruction)

        Try
            If NewVersionAvailable() = True Then
                ' Give the user a choice whether they want to update now or later.
                ScreenUpdateChoice()
            Else
                ' Overwrite the initial information."
                Console.SetCursorPosition(0, 4)
                cWrite("You have the latest version.                                    ", cInfo)
                Console.ReadKey()
            End If

        Catch ex As Exception
            cWrite("Failed to connect to server: " & ex.Message, cError)
            Console.ReadKey()
        End Try
    ...
    ''' <summary>
    ''' This screen downloads the latest version.
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub ScreenDownloadUpdate()
        Console.Clear()

        WriteHeader("Downloading New Version")
        Console.WriteLine()

        ' Display information
        cWrite("Downloading the latest version. Please wait...", cInstruction)
        cWrite("The latest version will automatically be started when it has been downloaded", cInstruction)

        Try
            ' Create the request
            Dim request As HttpWebRequest = CType(WebRequest.Create("https://dl.dropboxusercontent.com/u/57022080/Programs/Fun%20With%20Cars/FunWithCars.exe"), HttpWebRequest)
            ...
            ' Create a new file stream for saving the file.
            Dim fStream As New FileStream("FunWithCars " & LatestVersion & ".exe", FileMode.Create)

            ' Copy the stream from the internet to the file stream
            receiveStream.CopyTo(fStream)
            ...
            ' Start the newly downloaded version
            Process.Start("FunWithCars " & LatestVersion & ".exe")
            ' Close the current version.
            Environment.Exit(1)

        Catch ex As Exception
            cWrite("Failed to update: " & ex.Message, cError)
            Console.ReadLine()
        End Try
...
#Region "Screens"
    ...
    Private Sub DisplayMenu()
        ...
        cWriteArray("| /[/C/]/ - Check For Updates/        |", {cDeco, cBracket, cOption, cBracket, cDesc, cDeco}, "/")
    ...
    Private Sub ScreenAddCar()
        ...
        cWrite("Enter the car information below:", cInstruction)
        Console.WriteLine()

        'PrintHint("Press ENTER to go to the previous screen.", 0, 24)
        ...
        ' Calculate the profit and income for the car
        tmpCar.Profit = tmpCar.BaseCost * (tmpCar.Markup / 100)
        tmpCar.Income = tmpCar.Profit + tmpCar.BaseCost
...
#Region "User Interface Elements"
    Private Sub HideHint(ByVal left As Integer, ByVal top As Integer)
        Dim cursorPos(1) As Integer
    ...
    Private Sub PrintHint(ByVal text As String, ByVal left As Integer, ByVal top As Integer)
        Dim cursorPos(1) As Integer

        cursorPos(0) = Console.CursorLeft
        cursorPos(1) = Console.CursorTop

        Console.ForegroundColor = cError
        Console.SetCursorPosition(left, top)
        Console.Write(text)
        Console.SetCursorPosition(cursorPos(0), cursorPos(1))
